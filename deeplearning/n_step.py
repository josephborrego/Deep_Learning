...
# more complicated because we have to keep track of the most recent rewards, states, and actions
...
def play_one(model, eps, gamma, n=5):
 ...
 # multiplier is used for the n-step calculation
 # inside the loop we sample an action and perform the action as usual
 # but now we also have to keep track of state's actions and rewards when it comes time to do the update
 # we need to make sure we've collected at least (n) rewards
 # we then mutliply our last 10 rewards by the mutliplier we created earlier and then add this to 
 # the rest of the value prediction
 
 # if we hit the goal then any subsequent reward could be considered zero
 # if we haven't hit the goal then we have to make the assumption that we wouldn't hit the goal in the next steps
 # and so we set any subsequent rewards to -1
 mutliplier = np.array([gamma] * n) ** np.arange(n)
 # while not done and iters < 200:
 while not done and iters < 10000:
  # in earlier versions of gym, episode doesn't automatically end when you git 200 steps
 ...
 if observation[0] >= 0.5:
  ...
  # print("made it")
   ...
   while len(rewards) > 0:
	...
	model.update(status[0], actions[0], G)
	rewards.pop(0)
	states.pop(0)
	actions.pop(0)
 else:
...
if __name__ == '__main__':
 ...
 if 'monitor' in sys.argv:
  filename = os.path.basename(__file__).splot('.')[0]
